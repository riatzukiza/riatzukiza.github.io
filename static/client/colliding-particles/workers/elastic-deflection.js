...
var ElasticDeflectionSystem = ParentSystem.define("ElasticDeflectionSystem", { 
  ...
  bounds:[ minX, minY, maxX, maxY ]
    ...
    const threshold=(0.3 * (affector.scale + target.scale));;
    if( threshold > usedDistance ){ 
      ...
      if( target.scale < affector.scale ){ 
        ...
        correction.setLength((2 * (threshold - usedDistance)));
        correction.addTo(target.vel);
        correction.setAngle(dist.getAngle());
      ...
      const u1=Vector.spawn((((v1.x * (m - m_)) / (m + m_)) + (v2.x * (m_ / (m + m_)))), v1.y).rotateTo(theta);;
